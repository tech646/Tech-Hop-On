...
export default function AdminDashboard({
  ...
  const countries = useMemo(() =>
    [...new Set(students.map(s => s.country).filter(Boolean))].sort(), [students])
  const cities = useMemo(() =>
    [...new Set(students.map(s => s.city).filter(Boolean))].sort(), [students])
  ...
  function markAsTeacher(student: Student) {
    setLoadingId(student.id)
    startTransition(async () => {
      await setUserRole(student.id, 'teacher')
      // Move from students to teachers list (optimistic)
  ...
  function markAsStudent(teacher: Teacher) {
    setLoadingId(teacher.id)
    startTransition(async () => {
      await setUserRole(teacher.id, 'student')
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-colors ${
            tab === 'students' ? 'bg-[#0057b8] text-white' : 'bg-white border border-[#e1e7ef] text-[#65758b] hover:bg-[#f3f5f7]'
          }`}
        >
          <Users size={15} />
          Students
          <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${tab === 'students' ? 'bg-white/20 text-white' : 'bg-[#f3f5f7] text-[#65758b]'}`}>
            {students.length}
          </span>
        </button>
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-colors ${
            tab === 'teachers' ? 'bg-[#1f2c47] text-white' : 'bg-white border border-[#e1e7ef] text-[#65758b] hover:bg-[#f3f5f7]'
          }`}
        >
          <GraduationCap size={15} />
          Teachers
          <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${tab === 'teachers' ? 'bg-white/20 text-white' : 'bg-[#f3f5f7] text-[#65758b]'}`}>
            {teachers.length}
          </span>
        </button>
      </div>
        ...
        <>
          ...
          <div className="bg-white border border-[#e1e7ef] rounded-2xl p-5">
            ...
            {hasFilters && (
              ...
              >
                ✕ Clear filters · showing {filtered.length} of {students.length} students
              </button>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="bg-white border border-[#e1e7ef] rounded-2xl p-5">
              ...
              {countryBar.length > 0 ? (
                <ResponsiveContainer width="100%" height={220}>
                  <PieChart>
                    <Pie data={countryBar} cx="50%" cy="50%" innerRadius={55} outerRadius={85}
                      dataKey="value" nameKey="name" paddingAngle={2}>
                      {countryBar.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                    </Pie>
                      ...
                      <tr key={s.id} className="border-b border-[#f3f5f7] last:border-0 hover:bg-[#f9fafb] transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <button
                            ...
                            onClick={() => markAsTeacher(s)}
                            className="flex items-center gap-1.5 text-xs text-[#65758b] border border-[#e1e7ef] px-2.5 py-1.5 rounded-lg hover:border-[#1f2c47] hover:text-[#1f2c47] transition-colors disabled:opacity-50 whitespace-nowrap"
                          >
                            <GraduationCap size={12} />
                            {loadingId === s.id ? '...' : 'Mark as Teacher'}
                          </button>
                        </td>
                      </tr>
                    ))}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div className="bg-white border border-[#e1e7ef] rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-8 h-8 rounded-lg bg-[#1f2c47]/10 flex items-center justify-center">
              <GraduationCap size={16} className="text-[#1f2c47]" />
            </div>
            <span className="text-[10px] font-bold text-[#65758b] uppercase tracking-wider">TEACHERS</span>
          </div>
          <p className="text-3xl font-bold text-[#1b2232]">{teachers.length}</p>
        </div>
        <div className="bg-white border border-[#e1e7ef] rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-8 h-8 rounded-lg bg-[#22c55e]/10 flex items-center justify-center">
              <Calculator size={16} className="text-[#22c55e]" />
            </div>
            <span className="text-[10px] font-bold text-[#65758b] uppercase tracking-wider">TOTAL MATH CLASSES</span>
          </div>
          <p className="text-3xl font-bold text-[#1b2232]">{totalMath}</p>
        </div>
        <div className="bg-white border border-[#e1e7ef] rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-8 h-8 rounded-lg bg-[#0057b8]/10 flex items-center justify-center">
              <Calculator size={16} className="text-[#0057b8]" />
            </div>
            <span className="text-[10px] font-bold text-[#65758b] uppercase tracking-wider">AVG MATH / TEACHER</span>
          </div>
          <p className="text-3xl font-bold text-[#1b2232]">
            {teachers.length > 0 ? Math.round(totalMath / teachers.length) : '—'}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#e1e7ef] rounded-2xl overflow-hidden">
        ...
        {teachers.length === 0 ? (
          <div className="px-6 py-16 text-center">
            <GraduationCap size={32} className="text-[#e1e7ef] mx-auto mb-3" />
            <p className="text-[#65758b] text-sm">No teachers marked yet.</p>
            <p className="text-[#a0aec0] text-xs mt-1">Go to Students tab and click &quot;Mark as Teacher&quot; on a user.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {teachers.map(t => (
                  <tr key={t.id} className="border-b border-[#f3f5f7] last:border-0 hover:bg-[#f9fafb] transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-full bg-[#1f2c47]/10 overflow-hidden shrink-0 flex items-center justify-center text-sm">
                          {t.avatarUrl
                            ? <Image src={t.avatarUrl} alt={t.name} width={32} height={32} className="w-full h-full object-cover" unoptimized />
                            : <GraduationCap size={14} className="text-[#1f2c47]" />}
                        </div>
                        <div>
                          <p className="font-medium text-[#1b2232]">{t.name}</p>
                          {t.email && <p className="text-xs text-[#65758b]">{t.email}</p>}
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-[#65758b]">
                      <div>{t.country}</div>
                      {t.city && <div className="text-xs">{t.city}</div>}
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        onClick={() => onMarkAsStudent(t)}
                        className="flex items-center gap-1.5 text-xs text-[#65758b] border border-[#e1e7ef] px-2.5 py-1.5 rounded-lg hover:border-red-300 hover:text-red-500 transition-colors disabled:opacity-50 whitespace-nowrap"
                      >
                        {loadingId === t.id ? '...' : 'Remove Teacher Role'}
                      </button>
                    </td>
                  </tr>
                ))}
